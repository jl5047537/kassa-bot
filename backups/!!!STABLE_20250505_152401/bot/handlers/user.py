...
async def handle_contact(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        is_valid, error_message = validate_phone(contact.phone_number)
        if not is_valid:
            await message.answer(
                f"❌ {error_message}\n"
        ...
        # Проверяем, является ли пользователь предустановленным
        preset_user = db.get_preset_user_by_phone(contact.phone_number)
        if preset_user:
            if not preset_user.is_active:
                await message.answer(
            ...
            # Если пользователь не главный администратор, продолжаем обычную регистрацию
