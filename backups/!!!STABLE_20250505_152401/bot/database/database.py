...
class Database:
    ...
    def is_main_admin(self, user_id: int) -> bool:
        ...
        try:
            ...
            logging.error(f"Ошибка при проверке главного администратора: {e}")
            return False
        finally:
            session.close()
    ...
    def get_preset_users(self) -> List[PresetUser]:
        ...
        """
        try:
            logger.info("Начало выполнения get_preset_users")
            session = self.Session()
            logger.info("Сессия БД создана")
            
            query = session.query(PresetUser).filter(PresetUser.level.in_([1, 2, 3, 4])).order_by(PresetUser.level)
            ...
            logger.error(f"Ошибка в get_preset_users: {str(e)}", exc_info=True)
            raise
    ...
    def update_preset_user(self, user: PresetUser) -> bool:
        ...
        session = self.Session()
        try:
            logger.info(f"Обновление данных пользователя уровня {user.level}: "
                       f"username={user.username}, first_name={user.first_name}, last_name={user.last_name}")
            session.merge(user)
            session.commit()
    ...
    def get_admin_by_phone(self, phone_number: str) -> Optional[Admin]:
        """
        ...
        Returns:
            Optional[Admin]: Администратор или None, если не найден
        """
        try:
            return self.Session().query(Admin).filter(Admin.phone_number == phone_number).first()
        except Exception as e:
            logger.error(f"Ошибка при получении администратора по номеру {phone_number}: {str(e)}")
            return None
