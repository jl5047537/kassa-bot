...
def register_admin_handlers(dp: Dispatcher) -> None:
    ...
    Args:
        dp: Диспетчер бота
    """
    # Регистрация обработчика для входа в админ-панель
    dp.register_message_handler(
        show_admin_panel,
        lambda msg: msg.text == "/admin" or msg.text == "Админ" or msg.text == "admin",
        state="*"
    )
    
    # Регистрация обработчиков кнопок
    dp.register_message_handler(show_admins_list, lambda msg: msg.text == "👥 Список Админов")
    dp.register_message_handler(add_admin_prompt, lambda msg: msg.text == "✅ Добавить Админ")
    dp.register_message_handler(remove_admin_prompt, lambda msg: msg.text == "❌ Удалить Админ")
    dp.register_message_handler(edit_levels, lambda msg: msg.text == "🔍 Посмотреть Уровни")
    dp.register_message_handler(edit_levels_menu, lambda msg: msg.text == "✏️ Редактировать Уровни")
    
    # Регистрация обработчика обновления с приоритетом и для всех состояний
    dp.register_message_handler(
        handle_refresh_admin,
        lambda msg: msg.text == "🔄 Обновить",
        state="*",  # Обрабатывать во всех состояниях
        run_task=True  # Запускать в отдельном потоке
    )
    ...
    # Регистрация обработчика для удаления администратора
    dp.register_callback_query_handler(
        process_admin_remove,
        lambda c: c.data.startswith("remove_admin_"),
        state="*"
    )
